...
# from experiments.exp_financial_an import Exp_financial
import argparse
import pandas as pd
import numpy as np
...
parser.add_argument('--use_multi_gpu', action='store_true', help='use multiple gpus', default=None)
# parser.add_argument('--use_multi_gpu', action='store_true', help='use multiple gpus', default=[0,1,2,3,4,5,6,7])
# parser.add_argument('--use_multi_gpu', action='store_true', help='use multiple gpus', default=[0,1,2,3,4,5])
# parser.add_argument('--use_multi_gpu', action='store_true', help='use multiple gpus', default=[0,1])
# parser.add_argument('--use_multi_gpu', action='store_true', help='use multiple gpus', default=[0,1,2,3,4])
...
parser.add_argument('--pretrain_path', type=str, default='/home/gkdlfnddy/project/time/supplementary/IRN/checkpoint/electricity/seq24/bestmodel.pth')



### -------  model settings --------------  
parser.add_argument('--hidden-size', default=1.0, type=float, help='hidden channel of module')# H, EXPANSION RATE
parser.add_argument('--IRN', default=1, type=int, help='use INN or basic strategy')
...
if __name__ == '__main__':
    ...
    if args.evaluate:
        ...
        if args.stacks == 1:
            rse, rae, correlation,mape = exp.validate(data,data.test[0],data.test[1], evaluate=True)
        else:
            rse, rae, correlation, rse_mid, rae_mid, correlation_mid,mape = exp.validate(data,data.test[0],data.test[1], evaluate=True)
