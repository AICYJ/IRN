...
from utils.math_utils import smooth_l1_loss,MAPE
...
#long
# from models.SCINet_etth_eval_gr_6_dilation_res_etth2_720_relu_long import SCINet
...
class Exp_financial(Exp_Basic):
    ...
    def train(self):
        ...
        if self.args.resume:
            ...
            epoch_start = 0
        if self.args.use_multi_gpu is not None:
            self.model=nn.DataParallel(self.model,device_ids=self.args.use_multi_gpu).cuda()
        ...
        for epoch in range(epoch_start, self.args.epochs):
            ...
            for tx, ty in data.get_batches(X, Y, self.args.batch_size, True):
                if self.mix:
                    if self.mixon==0:
                        self.index_c=list(range(0,tx.shape[2]))
                        random.shuffle(self.index_c)
                        print(self.index_c)
                        self.mixon=1
                        for i in range(tx.shape[2]):
                            self.index_o.append(self.index_c.index(i))

                    for idx,i in enumerate(self.index_c):
                        if idx==0:
                            new_x=tx[:,:,i].reshape(tx.shape[0],tx.shape[1],1)
                        else:
                            new_x=torch.cat((new_x,tx[:,:,i].reshape(tx.shape[0],tx.shape[1],1)),dim=2)
                    tx=new_x
                self.model.zero_grad()             #torch.Size([32, 168, 137])
                if self.args.stacks == 1:
                    forecast,mma = self.model(tx)
                elif self.args.stacks == 2: 
                    forecast, res,mma = self.model(tx)
                # print(forecast.shape)
                # print(len(self.index_o))
                if self.mix:
                    for idx,i in enumerate(self.index_o):
                        if idx==0:
                            new_x_o=forecast[:,:,i].reshape(forecast.shape[0],forecast.shape[1],1)
                        else:
                            new_x_o=torch.cat((new_x_o,forecast[:,:,i].reshape(forecast.shape[0],forecast.shape[1],1)),dim=2)
                    forecast=new_x_o
                # print(forecast.shape)
                ...
                iter += 1
            if self.args.stacks == 1:
                val_loss, val_rae, val_corr ,mape= self.validate(data, data.valid[0],data.valid[1])
                test_loss, test_rae, test_corr,mape = self.validate(data, data.test[0],data.test[1])      
            elif self.args.stacks == 2:
                val_loss, val_rae, val_corr, val_rse_mid, val_rae_mid, val_correlation_mid,mape=self.validate(data, data.valid[0],data.valid[1])
                test_loss, test_rae, test_corr, test_rse_mid, test_rae_mid, test_correlation_mid,mape= self.validate(data, data.test[0],data.test[1])
            ...
            if test_loss < best_val:
                ...
                f = open(log_path, 'a')
                result='epoch{3}: res:{0} rae:{1} corr: {2} mape: {4}\n'.format(str(test_loss),str(test_rae),str(test_corr),str(epoch),str(mape))
                f.write(result)
                f.close()
                best_model_path=os.path.join(save_path,'bestmodel.pth')
                torch.save(self.model,best_model_path)
                print('--------------| Best Val loss |--------------')
                best_val = test_loss
            else:
                log_path=os.path.join(save_path, 'log_normal.txt')
                f = open(log_path, 'a')
                result='epoch{3}: res:{0} rae:{1} corr: {2} mape: {4}\n'.format(str(test_loss),str(test_rae),str(test_corr),str(epoch),str(mape))
                f.write(result)
                f.close()
    ...
    def validate(self, data, X, Y, evaluate=False):
        ...
        if evaluate:
            #model load
            save_path=self.args.pretrain_path
            print(save_path)
        ...
        for X, Y in data.get_batches(X, Y, self.args.batch_size, False):
            with torch.no_grad():
                ...
                if self.args.stacks == 1:
                    forecast, mma = self.model(X)
                
                elif self.args.stacks == 2:
                    forecast, res,mma = self.model(X) #torch.Size([32, 3, 137])\
        ...
        if self.args.stacks == 2:
            return rse, rae, correlation, rse_mid, rae_mid, correlation_mid,mape
