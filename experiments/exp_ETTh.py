...
class Exp_ETTh(Exp_Basic):
    ...
    def valid(self, valid_data, valid_loader, criterion):
        ...
        for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(valid_loader):
            pred, pred_scale, mid, mid_scale, true, true_scale,mma = self._process_one_batch_SCINet(
    ...
    def train(self, setting):
        ...
        for epoch in range(epoch_start, self.args.train_epochs):
            ...
            print("Epoch: {0}, Steps: {1} | Train Loss: {2:.7f} valid Loss: {3:.7f} Test Loss: {4:.7f}".format(
                epoch + 1, train_steps, train_loss, valid_loss, test_loss[0]))

            writer.add_scalar('train_loss', train_loss, global_step=epoch)
            writer.add_scalar('valid_loss', valid_loss, global_step=epoch)
            writer.add_scalar('test_loss', test_loss[0], global_step=epoch)
    ...
    def test(self, setting, evaluate=False):
        ...
        for i, (batch_x,batch_y,batch_x_mark,batch_y_mark) in enumerate(test_loader):
            ...
            # print(f'x_shape: {batch_x.shape}, mma_shape:{mma.shape}')
            if savecount==0:
                save_ori=batch_x
                save_mma=mma
            elif savecount<10:
                save_ori=torch.cat([save_ori,batch_x],dim=0)
                save_mma=torch.cat([save_mma,mma],dim=0)
                # print(mma)
            elif savecount==11:
                save_path = os.path.join(path,'result_ori')
                print('save start!!@#!@#: {}'.format(save_path))
                save_path_mma = os.path.join(path,'result_mma')
    ...
    def _process_one_batch_SCINet(self, dataset_object, batch_x, batch_y):
        ...
        if self.args.stacks == 1:
            outputs,mma = self.model(batch_x)
        elif self.args.stacks == 2:
            outputs, mid,mma = self.model(batch_x)
        else:
            print('Error!')
        ...
        if self.args.stacks == 1:
            return outputs, outputs_scaled, 0,0, batch_y, batch_y_scaled,mma
        elif self.args.stacks == 2:
            return outputs, outputs_scaled, mid, mid_scaled, batch_y, batch_y_scaled,mma
        else:
            print('Error!')
